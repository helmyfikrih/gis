$(function () {
	var myTable = $("#table_kota").DataTable({
		...
		ajax: {
			url: `${base_url}kategori_developer/getList`,
			type: "POST",
			data: function (d) {
...
function edit(e) {
	...
	jQuery.ajax({
		...
		url: `${base_url}kategori_developer/getOne`,
		dataType: "json",
		success: function (e) {
			jQuery("#kota_id").val(e[0].kategori_developer_id);
			jQuery("#kota_name").val(e[0].kategori_developer_name);
			jQuery("#kota_nameOld").val(e[0].kategori_developer_name);
		},
	});
}
...
function deleteData(id) {
	Swal.fire({
		...
		if (result.value) {
			var myForm = $("#form")[0];
			$.ajax({
				url: `${base_url}kategori_developer/delete`,
				type: "POST",
				data: {
					id: id,
				},
