...
class Kategori_developer extends CI_Controller
{
    public function __construct()
    {
        ...
        // Model
        $this->load->model('Kategori_developer_model', 'kota');
        // Menu Access Role
    ...
    public function index()
    {
        ...
        $this->js_path = array(
            'pages/kategori_developer.js',

        );
        $data = array(
            'user_session' => $this->sessionData,
            'setting' => $this->systemSetting,
            'header_title' => "Kategori Developer",
        ...
        if ((!in_array($this->data['menu_allow'] . '_read', $this->data['user_allow_menu']))) {
            ...
            $this->template->load('default', 'kategori_developer/index', $data);
        }
    }
    ...
    public function getList()
    {
        ...
        foreach ($list as $field) {
            ...
            if (array_intersect(array($this->data['menu_allow'] . '_update'), $this->data['user_allow_menu'])) {
                $btnEdit = '<span><button type="button" class="btn btn-outline-info btn-sm" onclick="edit(\'' . ($field->kategori_developer_id) . '\')"><i class="fa fa-edit"></i> Edit</button></span>';
            }
            if (array_intersect(array($this->data['menu_allow'] . '_delete'), $this->data['user_allow_menu'])) {
                $btnDelete = '<span><button type="button" class="btn btn-outline-danger btn-sm" onclick="deleteData(\'' . ($field->kategori_developer_id) . '\')"><i class="fa fa-trash"></i> Delete</button></span>';
            }
            ...
            $row[] = $btn;
            $row[] = $field->kategori_developer_id;
            $row[] = $field->kategori_developer_name;
            $data[] = $row;
        }
    ...
    function save()
    {
        ...
        $data = array(
            'kategori_developer_name' => strtolower($kotaName),
        );
        $cond = array(
            'kategori_developer_id' => $kotaId
        );
        if ($kotaId) {
            if (!array_intersect(array($this->data['menu_allow'] . '_update'), $this->data['user_allow_menu'])) {
                $res = array(
                ...
                $msg = "Berhasil Mengubah Kategori Developer";
            }
        } else {
            if (!array_intersect(array($this->data['menu_allow'] . '_create'), $this->data['user_allow_menu'])) {
                ...
                $msg = "Berhasil Membuat Kategori Developer";
            }
        }
    ...
    function delete()
    {
        ...
        $data['cond'] = array(
            "kategori_developer_id" => $kota_id,
        );

        if ($this->kota->deleteData($data)) {
            $res = array(
                'is_success' => true,
                'message' => "Berhasil Menghapus Kategori Developer",
            );
        } else {
            $err = $this->db->error();
            $msg = $err["code"] . "-" . $err["message"];
